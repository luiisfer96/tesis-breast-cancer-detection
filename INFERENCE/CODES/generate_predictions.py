    ...
    for d in tqdm(data):
        ...
        for v in ['L-CC', 'L-MLO', 'R-CC', 'R-MLO']:
            if len(d[v]) == 0:
                ...
                if v[0] == 'L':
                    label = d['cancer_label']['left_malignant']
                else:
                    label = d['cancer_label']['right_malignant']
                malignant_label.append(label)

                # Mostrar en tiempo real
                print(f"🖼 {image_id:35} | Label: {label} | Pred: {pred[0][1]:.4f} | Clasificado como: {int(pred[0][1] > 0.5)}")
                import sys
                sys.stdout.flush()
